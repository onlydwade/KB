    ...
    <div class="content-inner">
        <div class="bread_box">
            <a-breadcrumb>
                ...
                <a-breadcrumb-item>评价报告</a-breadcrumb-item>
            </a-breadcrumb>
        </div>
        <evaluateFilter v-model="filterForm" @submit="filterSubmit"/>
        ...
        <evaluateDrawer ref="drawerRef" @success="getPage"/>
    </div>
</template>
<script setup>
import api                                       from '@/api/index';
import { message }                               from 'ant-design-vue';
import evaluateFilter                            from './components/evaluateFilter.vue'
...
const getPage = ()=>{
    ...
    loadding.value = true;
    api.investment.correlationPage(postData,'companyAppraise',45).then(res=>{
        if(res.code==200){
...
const builderFilter = ()=>{
    ...
    let likeParams = ['company_name','report_name'];
    params.forEach(key => {
...
<script>export default{name:'homeInvestmentInvestment/evaluate'}</script>
