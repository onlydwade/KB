	...
	<div class="page-project content-inner">
		<div class="bread_box">
			<a-breadcrumb>
				...
				<a-breadcrumb-item><a @click="router.back()">存量商机</a></a-breadcrumb-item>
...
const formData = ref({});
const getInfo = () => {
	api.customer.getQlmStockDetail(id, lockStatus).then(res => {
		if (res.code == 200 ) {
...
onMounted(() => {	 
	...
	formData.value.biddingAmount =route.query.biddingAmount;
	formData.value.expirationDate =route.query.expirationDate;
	formData.value.extractPeriod =route.query.extractPeriod;
