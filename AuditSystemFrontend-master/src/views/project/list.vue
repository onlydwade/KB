<template>
  <div class="page-project content-inner">
      ...
      <Title title="项目列表">
        <template #right>
          <a-space>
            ...
            <a-button  @click="openProjectModal" v-permission="['biz:project:batchDelete']" style="margin-right: 10px;border:1px solid #F99C34;color:#F99C34;">
              项目删除
            </a-button>
          </a-space>
        </template>
    ...
    <ProjectModalList ref="projectModalListRef" @success="getPage"/>
  </div>
</template>
<script setup>
...
import ProjectModalList from "./components/ProjectModalList.vue";
...
const exportProjectAchievement = () => {
  let postData = builderFilter();
  store.spinChange(1);
  api.project.getOpenMarkList(postData).then((res) => {
    store.spinChange(-1);
    let timestamp = new Date().getTime();
    dataToFile(res, "投标台账-" + timestamp + ".xlsx");
  });
};
const builderFilter = () => {
  let postData = {
  ...
  if (!filterForm.moreFilter && filterForm.searchKey) {
    ...
    postData.contentColumns = ["sourceName","projectName", "projectNo"];
  }
  if (filterForm.year) {
...
const projectModalListRef = ref(null);
const openProjectModal = () =>{
  projectModalListRef.value.open();
}
onMounted(() => {
  getPage();
});
