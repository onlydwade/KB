...
<template>
    <a-space :size="16" v-if="oaReady">
      ...
      <template v-if="(hasPermission(['biz:projectExtension:exitOfflinePass']))">
        <a-button :loading="loadding" v-if="!oaNewest.approvalStatus || oaNewest.approvalStatus==0" :disabled="projectInfo.processState===2" size="large" type="primary" @click="startOa(0)">提交</a-button>
      </template>
      <template v-else>
        <a-button size="large" type="primary" disabled >无提交权限</a-button>
      </template>
<!--      -->
...
<!--        <template v-if="(hasPermission(['biz:projectExtension:createOa'])&&showStr == 'false') || showStr == 'true'">-->
<!--            <a-button :loading="loadding" v-if="!oaNewest.approvalStatus || oaNewest.approvalStatus==0" :disabled="projectInfo.processState===2" size="large" type="primary" @click="startOa(0)">提交</a-button>-->
...
<!--            <a-button :loading="loadding" v-if="oaNewest.approvalStatus==3" size="large" type="primary" @click="startOa(-1)">发起OA审批已驳回，再次发起</a-button>-->
<!--            <a-button :loading="loadding" v-if="oaNewest.approvalStatus==4" size="large" type="primary" @click="startOa(0)">发起OA审批已废弃，重新发起</a-button>-->
<!--        </template>-->
<!--        <template v-else>-->
...
<!--            </template>-->
<!--            <a-button size="large" type="primary" v-if="(oaNewest.approvalStatus&&(hasPermission(['biz:projectExtension:createNewOa'])))">发起新OA审批</a-button>-->
<!--        </a-popconfirm>-->
...
<!--                            &lt;!&ndash; <a-descriptions-item label="审批结果">{{resoutParse(oaNewest.approvalResult)}}</a-descriptions-item> &ndash;&gt;-->
...
<!--                                </template>-->
<!--                                &lt;!&ndash; <template v-if="column.key === 'approvalResult'">-->
