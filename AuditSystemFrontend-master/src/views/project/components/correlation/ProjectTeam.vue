...
<template>
        ...
        <a-table :columns="columns" :data-source="list" :pagination="false" :scroll="{ x: '100%' }">
            ...
            <template #bodyCell="{ column, text, record, index }">
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.editType] : ''"
                    v-if="column.editType == 'userId'">
                    <UserSelect v-if="editData[index]" v-model="editData[index][column.editType]"
                        @change="userChange($event, editData[index])" :users="editData[index].user" />
                    <template v-else>
                        {{ text }}
                    </template>
                </a-form-item>
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.editType] : ''"
                    v-if="column.editType == 'deptName'">
                ...
                <template v-if="column.key === 'action'">
                    <template v-if="!editData[index]">
                        <a-button type="text" class="color-primary"
                            v-if="(!readOnly && hasPermission(['biz:projectTeam:edit'])) || (!readOnly && showStr == 'true')"
                            size="small" @click="edit(record, index)">编辑</a-button>
                        <a-button type="text" class="color-primary"
                            v-if="(!readOnly && (type != 'TUO' || list.length > 1) && hasPermission(['biz:projectTeam:delete'])) || (!readOnly && (type != 'TUO' || list.length > 1) && showStr == 'true')"
                            size="small" @click="del(record, index)">删除</a-button>
                        <a-button type="text" class="color-primary"
                            v-if="(!readOnly && hasPermission(['biz:projectTeam:clone'])) || (!readOnly && showStr == 'true')"
                            size="small" @click="clone(record, index)">克隆</a-button>
                    </template>
                    <a-space v-else>
                    ...
                    <a-table-summary-row>
                        <template v-for="(item, index) in columns" :key="index">
                            <a-table-summary-cell v-if="item.key != 'action'">
                                <a-form-item :required="item.required" label="" :name="['add', item.editType]">
                                    <UserSelect v-if="item.editType == 'userId'" v-model="editData['add'][item.editType]"
                                        @change="userChange($event, editData['add'])" :users="editData['add'].user" />
                                    <a-input disabled v-if="item.editType == 'deptName'"
    ...
    <div class="add_btn" @click="addVisible = true"
        v-if="(!readOnly && !addVisible && hasPermission(['biz:projectTeam:add'])) || (!readOnly && !addVisible && showStr == 'true')">
...
import { useCorrelation } from './ProCorrelation';
...
watchArray(list, (newList) => {
    emit('update:modelValue', newList.filter((item)=>{return item.roleType=='TUO_HOU_JUE_SE'}).length>0 ? 'finished' : null);
