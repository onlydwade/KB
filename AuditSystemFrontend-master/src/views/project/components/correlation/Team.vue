            ...
            <template #bodyCell="{ column, text, record, index }">
                ...
                <template v-if="column.key === 'action'">
                    <template v-if="!editData[index]">
                        ...
                        <a-button type="text" class="color-primary" v-if="!readOnly && (type != 'TUO' || list.length > 1)"
                        ...
                        <template v-for="(item, index) in columns" :key="index">
                            <a-table-summary-cell v-if="item.key != 'action'">
                                <a-form-item :required="item.required" label="" :name="['add', item.editType]">
                                    ...
                                    <a-select v-if="item.editType == 'roleKey'" v-model:value="editData['add'][item.editType]"
                                        class="w_full" placeholder="请选择" :options="dict.options(editData['add'].roleType)">
                                    </a-select>
                                </a-form-item>
