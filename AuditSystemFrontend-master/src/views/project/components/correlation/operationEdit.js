...
export function useCorrelation(projectId,key,filter){
    ...
    const list    = ref([]);
    const getList = async ()=>{
        const res  = await api.project.projectCorrelationList(projectId);
        if(res.code==200){
            list.value = (res.data || [])
        }else{
    ...
    const addSubmit  = ()=>{
        validate(async ()=>{
            ...
            let res = await api.project.projectCorrelationAdd(editData['add'],key);
            if(res.code==200){
    ...
    const editSubmit = async (row,index)=>{
        let res = await api.project.projectCorrelationEdit(editData[index],key);
        if(res.code==200){
            message.success('操作成功');
            getList();
    ...
    const del = (row,index)=>{
        if(row.id){
            Modal.confirm({
                ...
                content : '是否确认删除该条数据？',
                onOk() {
                    api.project.projectCorrelationDel(row.id,key).then(res=>{
                        if(res.code==200){
