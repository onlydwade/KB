    ...
    <a-form ref="formRef" layout="vertical" :model="editData">
        <a-table :columns="columns" :data-source="list" :pagination="false" :scroll="{ x: '100%' }">
            ...
            <template #bodyCell="{ column, text, record, index }">
                ...
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.dataIndex] : ''"
                    v-if="column.editType == 'currencyType'">
                    <a-select v-if="editData[index]" v-model:value="editData[index][column.dataIndex]" class="w_full"
                        placeholder="请选择" :options="[{ label: '货币', value: 1 }, { label: '非货币', value: 2 }]">
                    </a-select>
                    <template v-else>
                        {{ { 1: '货币', 2: '非货币' }[text] }}
                    </template>
                ...
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.dataIndex] : ''"
                    v-if="column.editType == 'companyNo'"
                ...
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.dataIndex] : ''"
                    ...
                    <a-date-picker v-if="editData[index]" v-model:value="editData[index][column.dataIndex]"
                        :disabled-date="disabledDate" class="w_full" valueFormat="YYYY-MM-DD 00:00:00" format="YYYY-MM-DD"
                        placeholder="请选择" />
                    <template v-else>
                    ...
                    <a-table-summary-row>
                        <template v-for="(item, index) in columns" :key="index">
                            <a-table-summary-cell v-if="item.key != 'action'">
                                <a-form-item v-if="item.editType == 'companyNo'" label="" :required="item.required"
                                    :name="['add', item.dataIndex]"
                                    :rules="[{ required: true, validator: editData['add']['type'] == 1 ? companyNoRule : companyNoRuleID }]">
                                    <a-input allowClear v-model:value="editData['add'][item.dataIndex]" placeholder="请输入" />
                                </a-form-item>
                                <a-form-item v-else label="" :required="item.required" :name="['add', item.dataIndex]">
                                    <a-select v-if="item.editType == 'type'" v-model:value="editData['add'][item.dataIndex]"
                                        class="w_full" placeholder="请选择"
                                        :options="[{ label: '企业法人', value: 1 }, { label: '自然人', value: 2 }]">
                                    </a-select>
                                    <a-select v-if="item.editType == 'currencyType'"
                                        v-model:value="editData['add'][item.dataIndex]" class="w_full" placeholder="请选择"
                                        :options="[{ label: '货币', value: 1 }, { label: '非货币', value: 2 }]">
                                    </a-select>

                                    <a-input-number v-if="item.editType == 'persent'"
                                    ...
                                    <a-date-picker v-if="item.editType == 'time'"
                                        v-model:value="editData['add'][item.dataIndex]" :disabled-date="disabledDate"
                                        class="w_full" valueFormat="YYYY-MM-DD 00:00:00" format="YYYY-MM-DD"
                                        placeholder="请选择" />
                                </a-form-item>
...
const {
    ...
    del,
    clone
} = useCorrelation(props.projectId, 'projectCompanyShareholder', { id: props.companyId, attr: 'companyId' });
