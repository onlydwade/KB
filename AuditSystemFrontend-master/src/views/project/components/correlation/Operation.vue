        ...
        <a-table :columns="columns" :data-source="list" :pagination="false" :scroll="{ x: '100%' }">
            ...
            <template #bodyCell="{ column, text, record, index }">
                ...
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.editType] : ''"
                    v-if="column.editType == 'operateYear'">
                    <a-date-picker v-if="editData[index]" v-model:value="editData[index][column.editType]" @change="() => {
                        editData[index].roleKey = null;
                    }" class="w_full" picker="year" valueFormat="YYYY" />
                    <template v-else>
                        {{ text }}
                    </template>
                </a-form-item>
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.editType] : ''"
                    v-if="column.editType == 'takeoverRate'">
                ...
                <a-form-item label="" :required="column.required" :name="editData[index] ? [index, column.editType] : ''"
                    v-if="column.editType == 'businessIncome'">
                    <a-input placeholder="请输入" v-if="editData[index]" v-model:value="editData[index][column.editType]" />
                    <template v-else>
                        {{ text }}
                    </template>
                            ...
                            <a-table-summary-cell v-if="item.key != 'action'">
                                <a-form-item label=""> <!-- :required="item.required" :name="['add',item.editType]" -->
                                    ...
                                    <a-date-picker v-if="item.editType == 'operateYear'"
                                        ...
                                        valueFormat="YYYY" />
                                    <a-input placeholder="请输入" v-if="item.editType == 'takeoverRate'"
                                        v-model:value="editData['add'][item.editType]" />
                                    <a-input placeholder="请输入" v-if="item.editType == 'businessIncome'"
                                        v-model:value="editData['add'][item.editType]" />
...
const columns = [
    ...
    {
        title: '收缴率（%）',
        dataIndex: 'takeoverRate',
        editType: 'takeoverRate',
        width: 150
    },
    {
        title: '主营收入（元）',
        dataIndex: 'businessIncome',
        editType: 'businessIncome',
        width: 150
    },
    {
        title: '主营外收入（元）',
        dataIndex: 'outerBusinessIncome',
        editType: 'outerBusinessIncome',
        width: 150
    },
    {
        title: '成本（元）',
    ...
    if (props.businessTypeStr == undefined || props.companyName == undefined || props.projectName == undefined) {
        api.project.projectExtensionInfo(props.projectId).then(res => {
            if (res.code == 200) {
                ...
                callBack && callBack()
                console.log('paramsData2', paramsData)
            }
        })
    }
    else {
