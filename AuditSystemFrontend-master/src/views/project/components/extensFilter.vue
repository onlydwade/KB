      ...
      <a-row :class="{ show_more_col: formData.moreFilter }">
        ...
        <a-col :span="6" class="more_col">
          <a-form-item label="状态" name="serviceStatus">
            <a-select v-model:value="formData.serviceStatus" class="w_full" placeholder="请选择" :options="serviceStatus">
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :span="6" class="more_col">
        ...
        <a-col :span="formData.moreFilter ? 18 : 18" class="text-right">
...
const formData = computed({
  get: () => {
    ...
    data.year = null;//ref(data.year || moment(new Date()).format("YYYY"));
...
const reset = (data) => {
  ...
  formData.value.year =  null;//moment(new Date()).format("YYYY");
