            ...
            <a-collapse-panel key="ht">
              ...
              <div class="padding_box">
                <a-row :gutter="24">
                  ...
                  <a-col :xxl="6" :lg="8" :sm="12">
                    <a-form-item label="是否计算业绩" name="isCountPerformance">
                      <a-select
                        :getPopupContainer="trigger => trigger.parentNode"
...
import {amountUnit, numFixed, throttle, dateFormat, TpWatermark} from "@/utils/tools";
import { Modal, message } from "ant-design-vue";
import { useDictStore } from "@/store/dict";
import { mainStore } from "@/store";
import EnterAchievement from "../correlation/EnterAchievement.vue";
...
import { handleTree, isMobile } from '@/utils/tools';
...
const oaStart = (type, temp) => {
  let postData = {
    ...
    stepCode: null,
    mainProcess: 1,
    detailUrl: `${window.location.origin}/#/innerPage/projectEnterEdit?id=${projectId.value}&isView=1`,
...
//   if (isMobile()) {
//     // router.push("/mobile/backlogInfo?id=" + projectId.value);
//     router.push("/mobile/backlogInfo?read=1&id=" + projectId.value + "&access_token=" + token);
//   }else{
...
if (isMobile()) {
    if (token == undefined) {
        router.push("/mobile/backlogInfo?id=" + id);
    } else {
        router.push("/mobile/backlogInfo?id=" + id + "&access_token=" + token);
    }
} else {
    if (route.query.access_token) {
        sessionStorage.setItem('backPath', route.fullPath.replace('&access_token=' + route.query.access_token, ''))
...
onMounted(() => {
  ...
  enterInfo.templateId = "192995d0af5d4acd7099239413faaf4a";
...
const getdepTInfo = () => {
  getLoginUser();
  console.log("获取到了getdepTInfo1")
  getDict();
  console.log("获取到了getdepTInfo2")
  getDept();
  console.log("获取到了getdepTInfo3")
  getPost();
  console.log("获取到了getdepTInfo4")
}
