  ...
  <div class="card_box" v-if="list.length">
    ...
    <div class="list_box" v-for="(item, idx) in list" :key="idx">
      <a-row>
        ...
        <a-col :span="8">
          <div class="money">持股 {{item.rate}}%</div>
        </a-col>
        <a-col :span="16" align="middle">
          <div class="money">认缴出资金额：￥{{ item.investmentAmount }}</div>
        </a-col>
      </a-row>
    </div>
  </div>
...
const getList =  () => {
  loadding.value = true;
  api.project.correlationList(props.projectId,'projectCompanyShareholder').then(res => {
    if (res.code == 200) {
        list.value = res.data || [];
    }
