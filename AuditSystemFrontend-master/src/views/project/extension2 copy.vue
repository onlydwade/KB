              ...
              <div class="container_main" style="background-color: rgba(255, 250, 240, 1);">
                <div class="container_font2">
                  <div class="titleValue">
                    <span>项目数11：</span> <span class="num">{{ formData.projectTotal || 0 }} 个</span>
                    <span>续签数：</span> <span class="num">{{ formData.renewTotal || 0 }} 个</span>
                    <span>重新投标数：</span> <span class="num">{{ formData.newBidTotal || 0 }} 个</span>
                    <span>完结数：</span> <span class="num">{{ formData.finishTotal || 0 }} 个</span>
                    <span>退场数：</span> <span class="num">{{ formData.exitTotal || 0 }} 个</span>
      ...
      <FullTable :loadding="loadding" :columns="data.columns" :dataSource="data.list">
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-if="column.key === 'serviceContent'">
            <div class="flex_box">
              <EllipsisTooltip class="flex_full" :content="(record.serviceContentStr || '') +
          ...
          <template v-if="column.key === 'contractExpireTime'">
            ...
            <span
              v-if="record.processState == 2 || record.processMode == 1 || record.processMode == 2 || record.processMode == 3"></span>
            <span v-else>
...
const projectTotal = () => {
  api.project.expandProjectTotal({}).then((res) => {
    if (res.code == 200) {
...
const builderFilter = () => {
  ...
  if (filterForm.moreFilter || filterForm.moreFilter2) {
    ...
    let inParams = ['businessType', 'serviceContent', 'regionId', 'companyId', 'operationTeamId', 'processState' , 'processMode','expandServiceStatus','expandTwoServiceStatus' ]
    ...
    inParams.forEach((key) => {
      ...
      if (filterForm['expandTwoServiceStatus'] && filterForm['expandTwoServiceStatus'].length > 0) {
        postData.inParams['expandTwoServiceStatus'] = filterForm['expandTwoServiceStatus']
      }else{
        postData.inParams['expandTwoServiceStatus'] =undefined;
      } 
...
const actions = (record, index) => {
  return [
    {
      text: '续签',
      show: record.serviceStatus == 'YI_DAO_QI' && (record.show == true ||record.attributorOrCreateShow == true || hasPermission(['biz:projectExtension:renew'])) &&!(record.processMode=='3' && ([1, 2].includes(record.processState)))  ,//&& [0, 3].includes(record.processState),
      click: () => {
    ...
    {
      text: '重新投标',
      show: record.serviceStatus == 'YI_DAO_QI' &&(record.show == true ||record.attributorOrCreateShow == true || hasPermission(['biz:projectExtension:newBid'])) &&!(record.processMode=='3' && ([1, 2].includes(record.processState)))  ,//&& [0, 3].includes(record.processState),
      click: () => {
    ...
    {
      text: '运营数据更新',
      show: (record.principalShow == true || hasPermission(['biz:projectExtension:operation'])) && record.isSyncOperation === 'SHI',
      click: () => {
    ...
    {
      text: '拓前信息回顾',
      show: (record.principalShow == true || hasPermission(['biz:projectExtension:review']))&& record.isSyncOperation === 'SHI' ,
      click: () => {
