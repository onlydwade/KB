  ...
  <div class="content-inner">
    <div class="bread_box">
      <a-breadcrumb>
        ...
        <a-breadcrumb-item>项目评估</a-breadcrumb-item>
      </a-breadcrumb>
    </div>
    <evaluationFilter v-model="filterForm" @submit="filterSubmit" />
    <div class="content-box_full">
      <Title title="项目评估列表"></Title>
      <FullTable
        :loadding="loadding"
      ...
      >
        <template #bodyCell="{ record, column, index }">
          <template v-if="column.key === 'projectNo'">
            <router-link
              :to="'/innerPage/extensionInfo?id=' + record.id + '&businessTypeStr=' + record.businessTypeStr + '&companyName=' + record.companyName + '&projectName=' + record.projectName + '&to=thpg'+ '&show=' + record.show"
              class="color-link"
            >
              {{ record.projectNo }}
            </router-link>
  ...
  setup() {
      ...
      return [
        {
          ...
          show: true,
          click: () => {
            router.push('/innerPage/extensionInfo?id=' + record.id + '&businessTypeStr=' + record.businessTypeStr + '&companyName=' + record.companyName + '&projectName=' + record.projectName + '&show=' + record.show + '&to=thpg');
          },
        },
      ];
    };
    const getPage = () => {
      ...
      if (filterForm.moreFilter) {
        ...
        params.forEach((key) => {
          if (filterForm['assessDeadline'] && filterForm['assessDeadline'] != -1) {
            filterData.params['assess.assessDeadline'] = filterForm['assessDeadline'];
          }
          if (filterForm[key] && filterForm[key] != -1 && key != 'assessDeadline') {
      ...
      if (!filterForm.moreFilter && filterForm.assessState) {
        filterData.params.assessState = filterForm.assessState;
      }
      if (filterForm.year) {
      ...
      loadding.value = true;
      api.project.projectAssessListPage(filterData).then((res) => {
        if (res.code == 200) {
