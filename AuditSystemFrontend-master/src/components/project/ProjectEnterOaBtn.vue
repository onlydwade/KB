...
<template>
  <a-space :size="16" v-if="oaReady">
      <div v-if="checkoachild == 1">
          ...
          <template v-if="hasPermission(['biz:projectEnter:createOa']) && myProject">
      ...
      <a-button size="large" type="primary" @click="stagingA" v-permission="['biz:projectEnter:save']"
          :loading="submitLoading"
          v-if="((oaNewest.approvalStatus && (oaNewest.approvalStatus == 0 || oaNewest.approvalStatus == 3 || oaNewest.approvalStatus == 4 || oaNewest.approvalStatus == 5 || oaNewest.approvalStatus == 10)) || !oaNewest.approvalStatus || isAdministrators) && isView != 1">暂存</a-button>
      <a-button size="large" type="primary" @click="submitA" v-permission="['biz:projectEnter:submit']"
          :loading="submitLoading"
...
const isAdministrators = ref(false);
const isView = ref(0);
...
onMounted(() => {
  ...
  isAdministrators.value = props.menuInfo.isAdministrators;
