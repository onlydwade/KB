...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements ISysDeptService {
    ...
    @Override
    public List<SysDept> getIntersectionParents(Long deptId1, Long deptId2) {
        List<SysDept> list = super.list(Wrappers.<SysDept>lambdaQuery().in(SysDept::getDeptId, new Long[]{deptId1, deptId2}));
        if (list.size() == 2 && StringUtils.isNotNull(list.get(0).getAncestors()) && StringUtils.isNotNull(list.get(1).getAncestors())) {
            ...
            if (!intersection.isEmpty()) {
                return deptMapper.selectList(Wrappers.<SysDept>lambdaQuery().in(SysDept::getDeptId, intersection.stream().map(s -> Long.parseLong(s.trim())).collect(Collectors.toList())));
            }
        }
     ...
     */
    @Override
    public SysDept selectDeptByRegionId(Long deptId) {
        SysDept sysDept = deptMapper.selectDeptById(deptId);
        if (!"CENG_JI".equals(sysDept.getDeptType()) || !com.google.common.collect.Lists.newArrayList(1,2).contains(sysDept.getLevel())) {
            boolean b = true;
            deptId = sysDept.getParentId();
            while (b) {
                sysDept = deptMapper.selectById(deptId);
                //不是区域继续上找
                if (!"CENG_JI".equals(sysDept.getDeptType()) || !com.google.common.collect.Lists.newArrayList(1,2).contains(sysDept.getLevel())) {
                    deptId = sysDept.getParentId();
                } else {
    ...
    public boolean checkDeptExistUser(Long deptId) {
        ...
        if (result <= 0) {
            result = deptMapper.hasChildByPostDeptId(deptId);
        }
        return result > 0;
    }
    ...
    public int insertDept(SysDept dept) {
        ...
        dept.setLevel(dept.getAncestors().split(",").length);
    ...
    public List<Long> getChildDeptIds(Long deptId) {
        List<SysDept> childDeptIds = deptMapper.getChildDeptIds(deptId);
        List<Long> result = childDeptIds.stream().map(SysDept::getDeptId).collect(Collectors.toList());
        result.add(0, deptId);
        return result;
    }
